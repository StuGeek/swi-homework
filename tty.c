...
//计算距离
int calTheDistance(int Hx, int Hy, int Fx, int Fy);
//计算最小距离的方向
...
//计算距离
int calTheDistance(int Hx, int Hy, int Fx, int Fy){
    if (map[Hy][Hx] == SNACK_FOOD) return 0;
    if (map[Hy][Hx] != BLANK_CELL) return 9999;
    else return abs(Hx - Fx) + abs(Hy - Fy);
}
...
char whereGoNext(int Hx, int Hy, int Fx, int Fy)
{
    ...
    distance[0] = calTheDistance(Hx - 1, Hy, Fx, Fy);
    distance[1] = calTheDistance(Hx + 1, Hy, Fx, Fy);
    distance[2] = calTheDistance(Hx, Hy - 1, Fx, Fy);
    distance[3] = calTheDistance(Hx, Hy + 1, Fx, Fy);
...
int main()
{
    ...
    {
        sleep(1);
        ch = whereGoNext(HEAD_X, HEAD_Y, foodX, foodY);
        switch (ch) {
            case 'A':
            case 'a':
                snakeMove(-1, 0);
